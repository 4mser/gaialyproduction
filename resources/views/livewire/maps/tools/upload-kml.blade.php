<section>
    <button wire:click="setToggleModal()" class="flex w-16 w-full cursor-pointer flex-col items-center justify-center px-1 py-3 text-center hover:bg-gray-200">
        <img src="img/kml.svg" class="h-6 w-6" />
        <div class="mt-1 text-xs">{{ __('Upload KML/KMZ') }}</div>
    </button>
            ...
            <div class="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
                <form wire:submit.prevent="upload" enctype="multipart/form-data">
                    <div class="bg-white px-4 pb-4">
                        <div class="mt-3">
                            <h3 class="pb-2 text-center text-lg font-medium leading-6 text-gray-900" id="modal-title">
                                {{ __('Upload KML/KMZ') }}
                            </h3>
                            <div class="mt-2">
    ...
    <script>
        document.addEventListener("livewire:load", function() {
            Livewire.on('getGeom', (e) => {
                fetch(`storage/${e.route}`)
                    .then(res => res.text())
                    .then(kmltext => {
                        // Create new kml overlay
                        const parser = new DOMParser();
                        const kml_data = parser.parseFromString(kmltext, 'text/xml');
                        kml = new L.KML(kml_data).toGeoJSON();
                        kml = JSON.stringify(kml);
                        @this.updateData(e.id, kml);
                    });
            });
            Livewire.on('reload_kml', data => {
