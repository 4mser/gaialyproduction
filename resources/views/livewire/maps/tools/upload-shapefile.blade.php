<section>
    <button wire:click="setToggleModal()" class="flex w-16 w-full cursor-pointer flex-col items-center justify-center px-1 py-3 text-center hover:bg-gray-200">
        <img src="img/shape.svg" class="h-6 w-6" />
        <div class="mt-1 text-xs">{{ __('Upload SHP') }}</div>
    </button>
                    ...
                    <div class="bg-white px-4 pb-4">
                        <div class="mt-3">
                            <h3 class="flex items-center pb-4 text-lg text-xl font-medium leading-6 text-gray-900" id="modal-title">
                                <img src="img/shape.svg" class="mr-2 h-6 w-6" />
                                <span> {{ __('Upload SHP') }}</span>
                            </h3>
                            <div class="mt-2">
                                <div class="grid grid-cols-1">
                                    ...
                                    <div class="mb-4">
                                        <x-jet-label for="field" value="{{ __('Field to Show') }}" />
                                        <x-jet-input wire:model="tmpField" type="text" name="field" class="mt-1 block w-full" />
                                        <x-jet-input-error for="field" class="mt-2" />
                                    </div>
                                    <div class="mb-4">
                                        <x-jet-label for="file_shp" value="{{ __('File .shp') }}" />
                                        <x-jet-input wire:model="file_shp" type="file" name="file_shp" id="file-shp-{{ $iteration }}" class="block w-full cursor-pointer rounded-md border border-gray-300 p-1 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
                                        <x-jet-input-error for="file_shp" class="mt-2" />
                                    </div>
                                    <div class="mb-4">
                                        <x-jet-label for="file_shx" value="{{ __('File .shx') }}" />
                                        <x-jet-input wire:model="file_shx" type="file" name="file_shx" id="file-shx-{{ $iteration }}" class="block w-full cursor-pointer rounded-md border border-gray-300 p-1 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
                                        <x-jet-input-error for="file_shx" class="mt-2" />
                                    </div>
                                    <div class="mb-4">
                                        <x-jet-label for="file_dbf" value="{{ __('File .dbf') }}" />
                                        <x-jet-input wire:model="file_dbf" type="file" name="file_dbf" id="file-dbf-{{ $iteration }}" class="block w-full cursor-pointer rounded-md border border-gray-300 p-1 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
                                        <x-jet-input-error for="file_dbf" class="mt-2" />
                                    </div>
                                    <div class="mb-4">
                                        <x-jet-label for="file_prj" value="{{ __('File .prj') }}" />
                                        <x-jet-input wire:model="file_prj" type="file" name="file_prj" id="file-prj-{{ $iteration }}" class="block w-full cursor-pointer rounded-md border border-gray-300 p-1 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
                                        <x-jet-input-error for="file_prj" class="mt-2" />
                                    </div>
                                    <div class="mb-4">
    ...
    <div class="absolute" wire:loading wire:target="setToggleModal,upload,file_shp,file_shx,file_dbf,file_prj">
        <livewire:components.loading>
    </div>
    <script>
        document.addEventListener("livewire:load", function() {
            Livewire.on('reload_shp', data => {
